...
class Auth extends \CodeIgniter\Controller
{
    ...
    public function login()
    {
        ...
        if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
            ...
            if ($this->ionAuth->login($this->request->getVar('identity'), $this->request->getVar('password'), $remember)) {
                ...
                $this->session->setFlashdata('messagetype', 'danger');
            ...
            $this->data["user_registration"] = $admin->select(['user_registration'])->where('id_sorteio', 1)->first()["user_registration"];

            //google oauth2
            $this->data['google_button'] = $this->googleClient->createAuthUrl();
            ...
            // $_SESSION[''] = ;
            $this->data['facebook_button'] = $this->facebookClient->getAuthorizationUrl([
                'scope' => ['public_profile', 'email'],
            ]);
            //guarda variavel state enviado para fb oauth para comparar com state da resposta no metodo login_facebook
            $this->session->set("oauth2state", $this->facebookClient->getState());
    ...
    public function login_google()
    {
        ...
        if ($this->ionAuth->login_oauth($user_email)) {
            $this->session->setFlashdata('message', $this->ionAuth->messages());
            return redirect()->to('/dashboard')->withCookies();
        } else {
            $this->session->setFlashdata('message', $this->ionAuth->errors($this->validationListTemplate));
    ...
    public function login_facebook()
    {
        ...
        $token = $this->facebookClient->getAccessToken('authorization_code', [
            'code' => $_GET['code']
        ]);
        ...
        try {
            ...
            if ($this->ionAuth->login_oauth($user->getEmail())) {
                $this->session->setFlashdata('message', $this->ionAuth->messages());
                return redirect()->to('/dashboard')->withCookies();
            } else {
                $this->session->setFlashdata('message', $this->ionAuth->errors($this->validationListTemplate));
    ...
    public function logout()
    {
        ...
        // redirect them to the login page
        $this->session->setFlashdata('message', $this->ionAuth->messages());
        $this->session->setFlashdata('messagetype', 'success');
    ...
    public function forgot_password()
    {
        ...
        if (!($this->request->getPost() && $this->validation->withRequest($this->request)->run())) {
            ...
            if ($forgotten) {
                ...
                $this->session->setFlashdata('message', $this->ionAuth->messages());
                return redirect()->to('/dashboard'); //we should display a confirmation page here instead of the login page
            } else {
                $this->session->setFlashdata('message', $this->ionAuth->errors($this->validationListTemplate));
                return redirect()->to('/auth/forgot_password');
            }
    ...
    public function create_user()
    {
        ...
        $this->validation->setRule('telefone', lang('Auth.create_user_validation_phone_label'), 'trim');
    ...
    public function redirectUser()
    {
        if ($this->ionAuth->isAdmin()) {
            return redirect()->to('/auth/backoffice');
        }
        return redirect()->to('/dashboard');
    }
    ...
    public function edit_user(int $id)
    {
        ...
        if (!empty($_POST)) {
            ...
            //$this->validation->setRule('phone', lang('Auth.edit_user_validation_phone_label'), '');
            $this->validation->setRule('email', lang('Auth.edit_user_validation_email_label'), 'required|valid_email');
        ...
        $this->data['telefone'] = [
            ...
            'id'    => 'telefone',
            'type'  => 'text',
            'value' => set_value('telefone', empty($user->telefone) ? '' : $user->telefone),
    ...
    {
        $admin = new AdminModel();
        if ($admin->select(['user_registration'])->where('id_sorteio', 1)->first()["user_registration"]) {
            ...
            // validate form input
            $this->validation->setRule('first_name', lang('Auth.create_user_validation_fname_label'), 'trim|required|min_length[6]');

            $this->validation->setRule('email', lang('Auth.create_user_validation_email_label'), 'trim|required|valid_email|is_unique[' . $tables['users'] . '.email]');

            if ($this->request->getPost('telefone') != "")
                $this->validation->setRule('telefone', lang('Auth.create_user_validation_phone_label'), 'trim|numeric|min_length[9]');
